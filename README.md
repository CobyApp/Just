...
세 곳에만 배너가 붙습니다 — **곡을 준비하는 동안**, **퀴즈를 끝냈을 때**,
**오늘 복습을 끝냈을 때**. 셋 다 방금 일이 끝난 지점이라, 쓰던 것을 빼앗지
않습니다.

붙이지 않는 곳도 적어 둡니다. **가사와 플레이어**는 이 앱 자체이고, **문제가
떠 있는 화면**은 광고가 배우려는 것과 경쟁하며, **버튼에 붙여 놓는 것**은 AdMob
정책이 금하는 오클릭을 유도합니다. 세 곳 모두 버튼 위로 여백을 둡니다.

**전면 광고가 아닌 이유**는 그 대기가 온디바이스 모델이 일하고 있기 때문에
존재한다는 점입니다. 동영상 광고는 같은 CPU와 뉴럴엔진을 다투므로 채우려던 대기
시간을 오히려 늘리고 기기를 더 뜨겁게 만듭니다.

광고가 실제로 도착할 때까지 **높이 0**으로 둡니다. 빈 사각형을 미리 잡아 두면
채워지지 않을 수도 있는 자리를 위해 아트워크와 진행바를 밀어 올립니다 — 네트워크가
없거나 재고가 없거나 오프라인 첫 실행이면 영원히 비어 있습니다.
