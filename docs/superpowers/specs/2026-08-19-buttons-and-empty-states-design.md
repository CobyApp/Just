...
### 1. `JustIconButtonStyle` 신설

아이콘 전용 컨트롤을 위한 `ButtonStyle`. 최소 44×44 탭 영역을 보장하고,
`Surface.raised` 배경에 헤어라인 테두리를 둘러 컨트롤임을 알린다. 눌림
상태를 표시한다.

적용은 배경이 아예 없는 셋뿐이다 — 미니플레이어 재생/종료, 전체화면 재생.

**이미 의미 있는 배경을 가진 둘은 크기만 키운다.** 후리가나 토글은 켜짐/꺼짐을
배경 채움으로 말하고, 단어 담기 버튼은 저장됨을 초록 채움으로 말한다. 둘 다
토글로서 맞는 처리이므로 일반 스타일로 갈아치우면 그 정보를 잃는다. 34pt와
30pt를 44pt로 올리는 것이 전부다.
